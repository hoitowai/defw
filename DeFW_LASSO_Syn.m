...
% Simulation code for Fig. 6

addpath('./Tools_Data');

n = 1e4; 
m = 2e1; K = 50; % so we have altogether m*K obs.
sig = 0.1; 
p = 0.005; % the sparsity
no_iter = 1e3;
A = randn(m,n,K); % the obs matrix
theta = (rand(n,1)<p).*randn(n,1);
rho = 1.1*norm(theta,1); % the constraint param
% construct the obs
y = zeros(m,K);
for k = 1 : K
    y(:,k) = A(:,:,k)*theta + sig*randn(m,1);
end
load n1e4_syndata
...
for fw_iter = 1 : no_iter
    ...
    dt = min(n,1*ceil(2 + 0.05*(fw_iter)));
...
figure; semilogy( 1:no_iter, mean_extreme_obj,  1:no_iter, mean_rand_obj, ...
     1:no_iter, mean_dpg_obj,  1:no_iter, mean_extra_obj );
xlim([1e2,1e3]);
legend('DeFW (extreme)','DeFW (rand)','DPG','PG-EXTRA');
